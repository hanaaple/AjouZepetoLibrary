...
export default class ScreenShotModeManager extends ZepetoScriptBehaviour {
  ...
  public StartScreenShotMode() {
    ...
    let playerAnimator =
      this.localPlayer.character.gameObject.GetComponentInChildren<Animator>();
    if (this.iKController == null) {
      this.iKController =
        playerAnimator.gameObject.AddComponent<IKController>();
    }
    this.iKController.SetTargetAndGrip(target.transform, grip.transform);

    // 3. selfie stick을 캐릭터의 오른손에 고정
    if (this.selfieStick == null) {
      this.selfieStick = GameObject.Instantiate<GameObject>(
      ...
      this.localPlayer.character
        ...
        .forEach((characterObj) => {
          if (characterObj.name == this.rightHandBone) {
            ...
            this.selfieStick.transform.localRotation = Quaternion.Euler(
              Vector3.zero
            );
